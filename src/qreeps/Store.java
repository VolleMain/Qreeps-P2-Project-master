...
public class Store {
    ...
    public static int[i] buttonPrice = {10, 0, 0, 0, 0, 0, 0, 0};
                    ...
                    for(int y=0;y<Screen.room.block.length;y++) {
                        for(int x=0;x<Screen.room.block[0].length;x++) {
                            if(Screen.room.block[y][x].contains(Screen.mse)){
                                if(Screen.room.block[y][x].groundID != Value.groundRoad && Screen.room[y][x].airID == Value.airAir) {
    ...
    public void define() {
        ...
        buttonHealth = new Rectangle(Screen.room.block[0][0].x-1, button[o].y, iconSize, iconSize);
        buttonCoins = new Rectangle(Screen.room.block[0][0].x-1, button[o].y +button[0].height-iconSize, iconSize, iconSize);
    ...
    public void draw(Graphics g) {  
        ...
        if(holdsItem) {
            g.drawImage(Screen.tileset air[heldID], Screen.mse.x -((button[0].width -(itemIn*2))/2) +itemIn, Screen.mse.y -((button[0].width - (itemIn*2))/2) + itemIn, button[i].width - (itemIn*2), button[i].height - (itemIn*2), null);
        }
    }
}
